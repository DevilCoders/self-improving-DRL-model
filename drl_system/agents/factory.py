...
from .impala_agent import IMPALAgent
...
def create_agent(
    ...
    if agent_type == "reinforce":
        return REINFORCEAgent(obs_dim, action_dim, config.memory, config.ppo, device, agent_config)
    if agent_type == "impala":
        return IMPALAgent(obs_dim, action_dim, config.memory, config.ppo, device, agent_config)
    if agent_type == "rainbow":
        return RainbowAgent(obs_dim, action_dim, config.memory, config.ppo, device, agent_config)
