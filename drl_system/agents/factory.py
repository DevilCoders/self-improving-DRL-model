...
def create_agent(
    ...
    if agent_type == "impala":
        return IMPALAAgent(obs_dim, action_dim, config.memory, config.ppo, device, agent_config)
    if agent_type == "trpo":
        return TRPOAgent(obs_dim, action_dim, config.memory, config.ppo, device, agent_config)
