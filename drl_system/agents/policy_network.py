...
class ActorCritic(nn.Module):
    def __init__(
        ...
        self.temporal_projection = nn.Linear(last_dim, last_dim)
        self.representation_norm = nn.LayerNorm(last_dim)
        ...
        self.policy_embedding_head = nn.Sequential(
            nn.Linear(last_dim, last_dim),
            nn.LayerNorm(last_dim),
            nn.GELU(),
        )
        self.trajectory_value_head = nn.Sequential(
            nn.Linear(last_dim, last_dim),
            nn.GELU(),
            nn.Linear(last_dim, 1),
        )
        self.latent_drift_head = nn.Sequential(
            nn.Linear(last_dim, last_dim),
            nn.Tanh(),
        )
    ...
    def forward(self, obs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        temporal_signal = torch.tanh(self.temporal_projection(features))
        latent_drift = self.representation_norm(features - temporal_signal)
        features = features + 0.2 * latent_drift
        ...
        policy_embedding = self.policy_embedding_head(features)
        trajectory_value = self.trajectory_value_head(features)
        latent_drift_score = self.latent_drift_head(latent_drift)
        ...
        if squeezed:
            ...
            policy_embedding = policy_embedding.squeeze(0)
            trajectory_value = trajectory_value.squeeze(0)
            latent_drift_score = latent_drift_score.squeeze(0)
        ...
        diagnostics = {
            ...
            "latent_drift": latent_drift_score,
        }
            ...
            value,
                {
                    ...
                    "policy_embedding": diagnostics["policy_embedding"].detach(),
                    "trajectory_value": diagnostics["trajectory_value"].detach(),
                    "latent_drift": diagnostics["latent_drift"].detach(),
                },
            )
