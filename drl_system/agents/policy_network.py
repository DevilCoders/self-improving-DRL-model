...
class ActorCritic(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.quantile_atoms = max(8, int(quantile_atoms))
        ...
        self.quantile_head = nn.Sequential(
            nn.Linear(last_dim, last_dim),
            nn.GELU(),
            nn.Linear(last_dim, action_dim * self.quantile_atoms),
        )
    ...
    def forward(self, obs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        quantiles = self.quantile_head(features).view(-1, self.action_dim, self.quantile_atoms)
