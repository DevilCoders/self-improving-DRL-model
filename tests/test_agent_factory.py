...
def test_agent_factory_creates_impala():
    config = SystemConfig()
    config.agent.type = "impala"
    agent = create_agent(config.agent, config, obs_dim=4, action_dim=4, device=torch.device("cpu"))
    assert agent.__class__.__name__ == "IMPALAAgent"


def test_agent_factory_creates_trpo():
    config = SystemConfig()
    config.agent.type = "trpo"
    agent = create_agent(config.agent, config, obs_dim=4, action_dim=4, device=torch.device("cpu"))
    assert agent.__class__.__name__ == "TRPOAgent"
